...
def detect_datatype(source: Any) -> DataType:
    ...
    if url:
        ...
        if url.path.endswith(".md"):
            logger.debug(f"Source of `{formatted_source}` detected as `md`.")
            return DataType.MD
        ...
        if url.path.endswith(".doc"):
            logger.debug(f"Source of `{formatted_source}` detected as `doc`.")
            return DataType.DOC
        
        if url.path.endswith(".pptx") or url.path.endswith(".ppt"):
            logger.debug(f"Source of `{formatted_source}` detected as `ppt`.")
            return DataType.PPT
        ...
        if re.search(r'.(png|jpg|jpeg|bmp|tiff|tif)$', source):
            logger.debug(f"Source of `{formatted_source}` detected as `image`.")
